...
const schema = z.object({
  ...
  SESSION_COOKIE_NAME: z.string().regex(/^[a-zA-Z0-9_-]+$/).default('langgor_session'),
  SESSION_TTL_MS: positiveInt(43_200_000),
  REMEMBER_SESSION_TTL_MS: positiveInt(2_592_000_000),
  COOKIE_SECURE: bool(false),
  COOKIE_SAME_SITE: z.enum(['strict','lax','none']).default('lax'),
  BCRYPT_ROUNDS: z.preprocess(v => v === undefined || v === '' ? 12 : v, z.coerce.number().int().min(10).max(15)),
...
export const config = {
  ...
  auth: { sessionSecret: env.SESSION_SECRET, sessionCookieName: env.SESSION_COOKIE_NAME, sessionTtlMs: env.SESSION_TTL_MS, rememberTtlMs: env.REMEMBER_SESSION_TTL_MS, cookieSecure: env.COOKIE_SECURE, cookieSameSite: env.COOKIE_SAME_SITE, bcryptRounds: env.BCRYPT_ROUNDS },

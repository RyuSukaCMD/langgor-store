...
const loadAppUser=async(authUser:{id:string;email?:string|null}):Promise<AppUser>=>{
  const db=getSupabaseAdmin();const extended=await db.from('users').select('id,username,email,role,status,balance,created_at,profiles(nickname,bio,avatar_url,banner_url,accent,accent_secondary,profile_effect,profile_animation)').eq('id',authUser.id).single();let data:any=extended.data;let error=extended.error
  if(error){const legacy=await db.from('users').select('id,username,email,role,status,balance,created_at,profiles(nickname,bio,avatar_url,banner_url,accent)').eq('id',authUser.id).single();data=legacy.data;error=legacy.error}
  ...
  return{id:String(row.id),username:String(row.username),email:String(row.email||authUser.email||''),nickname,role:row.role,balance:Number(row.balance),avatar:initials,bio:String(profile.bio||''),joinedAt:String(row.created_at),accent:String(profile.accent||'#8b5cf6'),accentSecondary:String(profile.accent_secondary||'#22d3ee'),profileEffect:profile.profile_effect||'none',profileAnimation:profile.profile_animation||'fade',avatarUrl:profile.avatar_url||undefined,bannerUrl:profile.banner_url||undefined,status:row.status}
}
...
app.get('/api/users/:username/public',requireSupabase,async(req,res)=>{
  const db=getSupabaseAdmin();const username=String(req.params.username).toLowerCase();const extended=await db.from('users').select('username,created_at,profiles(nickname,bio,avatar_url,banner_url,accent,accent_secondary,profile_effect,profile_animation)').eq('username',username).eq('status','active').maybeSingle();let data:any=extended.data;let error=extended.error
  if(error){const legacy=await db.from('users').select('username,created_at,profiles(nickname,bio,avatar_url,banner_url,accent)').eq('username',username).eq('status','active').maybeSingle();data=legacy.data;error=legacy.error}
  if(error||!data)return res.status(404).json({message:'Profil tidak ditemukan.'});const profile=profileFromRelation(data.profiles) as any;res.json({profile:{username:data.username,nickname:profile.nickname||data.username,bio:profile.bio||'',avatarUrl:profile.avatar_url||null,bannerUrl:profile.banner_url||null,accent:profile.accent||'#8b5cf6',accentSecondary:profile.accent_secondary||'#22d3ee',profileEffect:profile.profile_effect||'none',profileAnimation:profile.profile_animation||'fade',joinedAt:data.created_at}})
})
...
app.patch('/api/profile',requireSupabase,requireAuth,async(req:AuthedRequest,res)=>{
  const body=parse(z.object({nickname:z.string().min(2).max(32),username:z.string().regex(/^[a-z0-9_]{4,20}$/),bio:z.string().max(160),accent:z.string().regex(/^#[0-9a-f]{6}$/i,'Warna utama tidak valid.'),accentSecondary:z.string().regex(/^#[0-9a-f]{6}$/i,'Warna kedua tidak valid.').default('#22d3ee'),profileEffect:z.enum(['none','aurora','stardust','comet','ripple','pixels']).default('none'),profileAnimation:z.enum(['fade','rise','zoom','slide','flip']).default('fade')}),req.body,res);if(!body||!req.user)return
  ...
  const profileUpdate=await db.from('profiles').upsert({user_id:req.user.id,nickname:body.nickname,bio:body.bio,accent:body.accent,accent_secondary:body.accentSecondary,profile_effect:body.profileEffect,profile_animation:body.profileAnimation},{onConflict:'user_id'});if(profileUpdate.error){if(before.data.username!==body.username)await db.from('users').update({username:before.data.username}).eq('id',req.user.id);console.error('[profile] customization update failed:',profileUpdate.error.message);return res.status(500).json({message:'Profil belum disimpan. Pastikan pembaruan profil terbaru sudah diterapkan.'})}
  const avatar=body.nickname.split(/\s+/).map(part=>part[0]).slice(0,2).join('').toUpperCase();res.json({user:{...req.user,username:body.username,nickname:body.nickname,bio:body.bio,accent:body.accent,accentSecondary:body.accentSecondary,profileEffect:body.profileEffect,profileAnimation:body.profileAnimation,avatar}})
})

...
const supabaseUnavailable=(res:Response)=>res.status(503).json({message:'Supabase belum dikonfigurasi. Isi environment Supabase lalu jalankan migration server/schema.sql.'})
...
const loadAppUser=async(authUser:{id:string;email?:string|null}):Promise<AppUser>=>{
  ...
  if(error||!data)throw new Error('Profil pengguna tidak ditemukan. Jalankan trigger handle_new_user di Supabase.')
...
app.patch('/api/profile',requireSupabase,requireAuth,async(req:AuthedRequest,res)=>{
  ...
  const profileUpdate=await db.from('profiles').update({nickname:body.nickname,bio:body.bio,accent:body.accent}).eq('user_id',req.user.id);if(profileUpdate.error){if(before.data.username!==body.username)await db.from('users').update({username:before.data.username}).eq('id',req.user.id);return res.status(500).json({message:'Profil belum disimpan.'})}
  const authUser=(await db.auth.admin.getUserById(req.user.id)).data.user;res.json({user:authUser?await loadAppUser(authUser):null})
})
...
async function replaceStoredImage(options:{table:'profiles'|'products';idColumn:'user_id'|'id';id:string;pathColumn:string;urlColumn:string;prefix:string;buffer:Buffer;contentType:string;ext:string}){
  ...
  else throw new Error('Record media tidak ditemukan. Jalankan migration media terbaru.')
  const oldPath=row[options.pathColumn]||storagePathFromUrl(row[options.urlColumn]||undefined)
  const newPath=`${options.prefix}/${crypto.randomUUID()}.${options.ext}`;const upload=await db.storage.from(config.supabase.storageBucket).upload(newPath,options.buffer,{contentType:options.contentType,upsert:false});if(upload.error)throw new Error(`Upload gagal: ${upload.error.message}`)
  ...
  const update=await db.from(options.table).update(patch).eq(options.idColumn,options.id);if(update.error){await db.storage.from(config.supabase.storageBucket).remove([newPath]);throw new Error(`Database gagal diperbarui: ${update.error.message}`)}
...
app.use((error:unknown,_req:Request,res:Response,_next:NextFunction)=>{const issue=error as {type?:string};if(issue?.type==='entity.too.large')return res.status(413).json({message:'Ukuran file melebihi batas 5MB.'});console.error(error);res.status(500).json({message:'Server sedang bermasalah. Coba beberapa saat lagi.'})})

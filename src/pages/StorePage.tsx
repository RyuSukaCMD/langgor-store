...
export function StorePage({ kind }: { kind: ProductKind }) {
  ...
  return <div className="cookie-shop">
    ...
    <main>
      ...
      <section className="shop-verify"><div className="container shop-verify__inner"><div><span className="game-kicker">SETELAH CHECKOUT</span><h2>Bayar selesai.<br/>Sistem langsung bekerja.</h2><p>Cookie dikirim hanya setelah pembayaran dan pemeriksaan selesai.</p><Link to="/register" className="btn btn--light">Buat akun <ArrowRight/></Link></div><div className="shop-verify__steps">{[[ShoppingBag,'Pembayaran','Sudah diperiksa'],[Radar,'Validasi','Real-time check'],[PackageCheck,'Delivery','Cookie tersedia']].map(([Icon,title,text],i)=><motion.article key={String(title)} initial={{opacity:0,y:15}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.12}}><span><Icon/></span><small>0{i+1}</small><strong>{String(title)}</strong><p>{String(text)}</p>{i<2&&<ArrowRight/>}</motion.article>)}</div></div></section>

      <section className="shop-help"><div className="container"><div><HelpCircle/><span><strong>Katalog tersinkron</strong><small>{selected?`${selected.name} sedang dipilih.`:'Pilih produk untuk melihat rinciannya.'}</small></span></div><div><ShieldCheck/><span><strong>100% valid.</strong><small>Cookie diperiksa sesaat sebelum dikirim.</small></span></div><div><TimerReset/><span><strong>Pengiriman otomatis.</strong><small>Delivery dibuka setelah validasi selesai.</small></span></div></div></section>
    </main>
  </div>
}

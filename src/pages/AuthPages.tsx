...
function AuthArt({ mode }: { mode: Mode }) {
  return <aside className="auth-art">
    ...
    <div className="auth-art__message">
      ...
      <h2>{mode === 'register' ? <>Buat akun.<br/>Lanjut ke <em>Cookie Store.</em></> : <>Masuk cepat.<br/>Order tetap <em>tercatat.</em></>}</h2>
      <p>Pembelian Cookie, status validasi, delivery, dan riwayat transaksi tersedia dalam satu dashboard.</p>
    </div>
...
export function AuthPage({ mode }: { mode: Mode }) {
  ...
  const subtitle = mode === 'login' ? 'Lihat pembelian, status validasi, dan delivery Cookie.' : mode === 'register' ? 'Satu akun untuk checkout dan mengakses seluruh riwayat pembelian.' : mode === 'forgot' ? 'Kami kirim tautan pemulihan jika email terdaftar.' : 'Gunakan kombinasi yang belum pernah kamu pakai.'
